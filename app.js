...
function playWebScanBeep() {
    ...
    const handleScanUpdate = (snapshot) => {
        ...
        if (typeof data === "object") {
            academicId = data.academicId || data.code || data.studentId || data.id;
            timestamp = Number(data.timestamp) || Date.now();
        } else if (typeof data === "string" || typeof data === "number") {
            academicId = String(data);
            timestamp = Date.now();
        }
        ...
        if (academicId) {
            ...
            if (timestamp > lastProcessedScanTime || (Date.now() - lastProcessedScanTime > 1200)) {
                lastProcessedScanTime = timestamp || Date.now();
                playWebScanBeep();
                ...
                if (activeTab === "students") {
                    ...
                    checkAndLoadStudent(academicId);
                    quickAttendStudentSession(academicId, currentSess);
                } else {
                    inputTeacherAcademicId.value = academicId;
                    ...
                    showToast("رمز QR جديد ممسوح", `الرقم الأكاديمي للمعلم: ${academicId}`, "info");
                    checkAndLoadTeacher(academicId);
                }
            }
        }
    };

    db.ref("scanned_code").on("value", handleScanUpdate);
    db.ref("scannedCode").on("value", handleScanUpdate);
...
function setupEventHandlers() {
    ...
    if (btnClearHistory) btnClearHistory.addEventListener("click", clearStudentHistory);
...
async function checkAndLoadStudent(academicId) {
    ...
    try {
        ...
        if (snapshot.exists()) {
            ...
            // Set buttons state for editing/deleting/clearing history
...
// Database Action: Clear Student Academic & Financial History (All Months)
async function clearStudentHistory() {
    const id = inputAcademicId ? inputAcademicId.value.trim() : "";
    const name = inputName ? inputName.value.trim() : "";

    if (!id) {
        showToast("تنبيه ⚠️", "يرجى اختيار أو البحث عن طالب أولاً لمسح سجله الأكاديمي والمالي.", "warning");
        return;
    }

    const studentName = name || (selectedStudent ? selectedStudent.name : id);

    const confirmed = await showConfirmModal({
        title: "مسح سجل الطالب بالكامل",
        message: `هل أنت متأكد من مسح جميع سجلات الحضور والغياب والدرجات والمصروفات لكافة الشهور للطالب: (${studentName})؟ سيتم الإبقاء على بياناته الأساسية فقط.`,
        confirmText: "مسح السجل الآن",
        type: "danger"
    });

    if (confirmed) {
        try {
            await db.ref(`students/${id}/records`).remove();

            // Update local state in memory
            const localStudent = allStudents.find(s => String(s.academicId).trim() === String(id).trim());
            if (localStudent) {
                localStudent.records = {};
            }
            if (selectedStudent && String(selectedStudent.academicId).trim() === String(id).trim()) {
            ...
            updateDashboardStats();
            updateAnalyticsDashboard();
            if (sectionStudentProfile && !sectionStudentProfile.classList.contains("hidden")) {
                renderMonthlyRecords();
            }

            showToast("تم مسح السجل بنجاح 🗑️", `تم مسح جميع سجلات الطالب (${studentName}) لجميع الشهور بنجاح مع الاحتفاظ ببياناته الأساسية.`, "success");
        } catch (error) {
            console.error("Failed to clear student records:", error);
            showToast("خطأ أثناء المسح 🔴", `فشل مسح السجل: ${error.message}`, "error");
        }
    }
}
            ...
            validTopStudents.forEach((student, index) => {
                ...
                item.innerHTML = `
                    ...
                    <div class="top-student-info">
                        <span class="top-student-name">${escapeHTML(student.name)}</span>
                        <div class="top-student-details">
                            <span class="detail-id"><i class="fa-solid fa-id-card"></i> ${escapeHTML(student.academicId)}</span>
                            <span class="detail-grade"><i class="fa-solid fa-school"></i> ${escapeHTML(student.grade)}</span>
                        </div>
                    </div>
